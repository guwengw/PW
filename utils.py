...
def make_data(dataset='mnist'):
    ...
    if dataset == "mushroom":
        x, t = load_svmlight_file(datapath+"mushrooms.txt")
        x = x.toarray()
        ...
        x_train, t_train = load_svmlight_file(datapath+'shuttle.scale.txt')
        x_train = x_train.toarray()
        x_test, t_test = load_svmlight_file(datapath+'shuttle.scale.t')
        ...
        x_train, t_train = load_svmlight_file(datapath+'usps')
        x_train = x_train.toarray()
        x_test, t_test = load_svmlight_file(datapath+'usps.t')
        x_test = x_test.toarray()
        x = np.concatenate([x_train, x_test])
        t = np.concatenate([t_train, t_test])
        t[t == 1] = 1
        ...
        x_train, t = load_svmlight_file(datapath+'mnist.scale')
        x = x_train.toarray()
        #t[(t == 2)] = 0 #1-4672  0-8320
        t[(t != 1)] = 0
